...
For each drug–disease pair, we use a **disease vector** $\mathbf{d} \in \mathbb{R}^{d_d}$ derived from protein embeddings or disease ontologies.

### Conditional GraphVAE

We use a **Conditional Variational Autoencoder (CVAE)** where both the encoder and decoder are conditioned on the disease vector $\mathbf{d}$.
...
Here $\mu_{\phi}$ and $\log \mathbf{{\sigma}_{\phi}}$ are generated by a **Graph Neural Network**. We concatenate the disease vector to each node feature: $\tilde{\mathbf{x}}_i = [\mathbf{x}_i; \mathbf{d}]$. After graph convolutions and global pooling, linear layers produce the distribution parameters.

#### Reparameterization

To allow backpropagation, we use the reparameterization trick:
...
Since graphs are unordered, we use the **Hungarian algorithm** to find the optimal permutation $\pi$ to align predicted and ground-truth nodes:
